...
public sealed class SchwabTradeImporter : ITradeImporter
{
    ...
    /// </summary>
    public TradeImportResult Import(string csv)
    {
        ...
        var unmatchedOrders = new List<UnmatchedOrder>();
                         ...
                         StringComparer.OrdinalIgnoreCase))
        {
            ...
            foreach (var order in symbolGroup)
            {
                ...
                // A SELL with no corresponding BUY is retained
                // for review rather than silently discarded.
    ...
    /// </summary>
    public TradeImportResult ImportFile(string filePath)
    {
    ...
    {
        var status =
            GetRequired(row, StatusHeader);

        var action =
            GetRequired(row, ActionHeader);

        var fillPrice =
            GetRequired(row, FillPriceHeader);
